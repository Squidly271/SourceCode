#!/usr/bin/php
<?php
########################  Check the branches set in Docker Templates if there's issues with downloading
...
$appPaths['recommendedURL']  = "https://raw.githubusercontent.com/Squidly271/Community-Applications-Moderators/master/Recommended.txt";
...
$appPaths['CAlanguageBase']  = "/tmp/GitHub/AppFeed/languages/";

exec("mkdir -p {$appPaths['CAlanguageBase']}");
#
...
function fixAttributes(&$template,$attribute) {
	...
	if ( $template[$attribute]['@attributes'] ) {
		...
		if ( isset($template[$attribute]['value']) ) {
...
// tag things as being beta if the tag contains "beta"
	...
	$template['Category'] = str_ireplace("Status:Stable","",$template['Category']);
	$template['Category'] = trim(preg_replace('!\s+!', ' ', $template['Category']));
	if ( $template['Private'] ) {
...
function languageErrors($o) {
	...
	if ( ! $countryCode || $countryCode == "en_US" ) 
		return;
	$languageErrors["$countryCode - {$o['Language']} / {$o['LanguageLocal']}"] = array();
...
foreach ($repositories as $repo) {
	...
	unset($repositoryBranch);
	for ($i=0;$i<5;$i++) {
		$downloaded = $DockerTemplates->downloadTemplates($localPath, $downloadURL,"master");
		if ( $downloaded ) {
			$repositoryBranch = "master";
			break;
		}
		$downloaded = $DockerTemplates->downloadTemplates($localPath, $downloadURL,"main");
		if ( $downloaded ) {
	...
	if ( ! $downloaded ) {
		...
		$repoList[$repo['name']] = array("url"=>$repo['url'],"profile"=>$repo['profile']);
		...
		foreach ($downloaded[$repo['url']] as $xmlFile) {
			...
			if ( basename($xmlFile) == "ca_profile.xml" ) {
				echo "FOUND PROFILE\n";
				if ( $o['Profile'] )
					$repoList[$repo['name']]['bio'] = trim($o['Profile']);
				if ( $o['Icon'] )
					$repoList[$repo['name']]['icon'] = trim($o['Icon']);
				if ( validURL($o['DonateLink']) ) {
				...
				if (validURL($o['Twitter'])) $repoList[$repo['name']]['Twitter'] = $o['Twitter'];
				if (validURL($o['Discord'])) $repoList[$repo['name']]['Discord'] = $o['Discord'];
				if (validURL($o['WebPage'])) $repoList[$repo['name']]['WebPage'] = $o['WebPage'];
				continue;
			}
			$subnet = array();
...
//			$o['Recommended'] = $repo['recommended'];
			...
			$repoBranch = $foundTree ? "" : "/$repositoryBranch";
...
// Save copy of what CA requires to translate language popups properly.  This gets downloaded when the popup is opened
				copy("{$appPaths['languageFiles']}/lang-$countryCode-master/Community Apps/apps.txt","{$appPaths['CAlanguageBase']}/$countryCode");
				file_put_contents("{$appPaths['CAlanguageBase']}/$countryCode",file_get_contents("{$appPaths['languageFiles']}/lang-$countryCode-master/docker.txt"),FILE_APPEND);
				file_put_contents("{$appPaths['CAlanguageBase']}/$countryCode",file_get_contents("{$appPaths['languageFiles']}/lang-$countryCode-master/translations.txt"),FILE_APPEND);

// Add to appfeed what is needed on the hover text for the language cards
				...
				$o['UpdateLanguage'] = getTranslation("Update Language Pack");
				
				$o['disclaimLineLink'] = "https://forums.unraid.net/topic/93770-unraid-webgui-translations-disclaimer/";
...
if ( $oldApps !== $newApps || $argv[1]) {
	$appFeed['apps'] = count($newApps);
	$endTime = time();
	$appFeed['last_updated_timestamp'] = $endTime;
	$appFeed['last_updated'] = date("Y-m-d H:i",$endTime);

	$appFeed['categories'] = $master_Categories;
	$appFeed['applist'] = $newApps;
	$appFeed['repositories'] = $repoList;

/* 	foreach ($repositories as $repo) {
		$repoList[$repo['name']] = array("url"=>$repo['url'],"profile"=>$repo['profile']);
	} */
	$lastUpdated['last_updated_timestamp'] = $endTime;
...
class DockerTemplates {
	...
	public function downloadTemplates($Dest=null, $Urls=null,$branch="master") {
		global $dockerManPaths;
		...
		foreach ($urls as $url) {
			...
			foreach ($github_api_regexes as $api_regex) {
				if (preg_match($api_regex, $url, $matches)) {
					...
					$github_api['branch'] = $matches[3] ?? $branch;
			...
			/// SAVE PROFILE.txt
			$profile = @file_get_contents("$tmp_dir"."/profile.txt");
			...
			if ($profile) {
				file_put_contents("$Dest/profile.txt",$profile);
			}
